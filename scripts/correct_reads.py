...
class CorrectReadCount(object):
    ...
    use fitted model to predict corrected gc and mappability values
    """
    ...
    def ideal(self, df):
        ...
        range_l, range_h = mquantiles(valid_reads, prob=[0, 1 - routlier], alphap=1, betap=1)
        domain_l, domain_h = mquantiles(valid_gc, prob=[doutlier, 1 - doutlier], alphap=1, betap=1)
    ...
    def create_dataframe(self, reads, mapp, gc):
        """merge data from reads, mappability and gc wig files into pandas dataframe
        
        Args:
            reads: list of tuples, formatted as [(chromosome, start, end, count), ]
            mapp: list of tuples, formatted as [(chromosome, start, end, mappability value), ]
            reads: list of tuples, formatted as [(chromosome, start, end, gc content), ]
        
        """
        err_str = 'please ensure that reads, mappability and gc wig files have the same sort order'
        ...
        for read_v, mapp_v, gc_v in zip(reads, mapp, gc):
            ...
            data.append((read_v[0], read_v[1], read_v[2], read_v[3], gc_v[4], mapp_v[4], read_v[4],))
    ...
    def modal_quantile_regression(self, df_regression, lowess_frac=0.2):
        ...
        df_dist_filter['lowess'] = lowess(df_dist_filter['distances'], 
    ...
    def main(self):
        ...
        df.loc[df_regression.index, 'modal_quantile'] = df_regression['modal_quantile']
...
def main(args):
    out_dir = Path(args.corr_read_counts).parent
    out_dir.mkdir(parents=True, exist_ok=True)
    corr = CorrectReadCount(args.gc, 
                            args.map, 
                            args.read_counts, 
                            args.corr_read_counts, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--corr-read-counts', help='path to the output corrected read-counts csv file', required=True)
    parser.add_argument('--read-counts', help='path to the read-counts wig file', required=True)
    parser.add_argument('--gc', help='path to the gc wig file', required=True)
    parser.add_argument('--map', help='path to the mappability wig file', required=True)
    parser.add_argument('--mappability', type=float, help='specify mappability threshold', default=0.9)
